...
def save_attachments(part, save_folder):
    ...
    if filename:
        ...
        if not os.path.exists(filepath):
            print("Downloadingss attachment:", filename)
            with open(filepath, 'wb') as f:
                f.write(part.get_payload(decode=True))
            # with open(filepaths, 'wb') as f:
            #     f.write(part.get_payload(decode=True))
        else:
            print("Attachmentss already downloaded:", filename)
...
def fetch_emails_Credential(data):
    print(data)
    # orgid = orgId[0]
    # with open(F"{orgid}/credential.json", "r") as file:
    #     my_credentials = json.load(file)

    # user = my_credentials["user"]
    # password = my_credentials["password"]
    # orgId = my_credentials['orgId']
    # mail = imaplib.IMAP4_SSL('imap.gmail.com')
    # mail.login(user, password)
    # mail.select('inbox')
    # save_folder = f"{orgId}/email"
