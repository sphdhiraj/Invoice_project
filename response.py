...
def getResponse(file_path,id):
    ...
    new_path =f"{id}/{final_name}"
    pdf_format_name = list(collection.find({"name": new_path}))


    # using for mistral code 
    # format_id = get_format_identifiers(file_name)
    # print(format_id)
    # docs = list(collection.find({"name": format_id}))
    # print(docs)
    
    for res in pdf_format_name:
            document_data_res = {
                        "type": res['type'],        
